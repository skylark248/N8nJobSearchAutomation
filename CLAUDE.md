...
- **v9.13** (2026-05-09): Fixed "Bad request" 400 error in Search Naukri (Apify) — root cause: v9.12 set `experience` body parameter as string `"4"` instead of integer `4`. The Apify naukri-job-scraper actor's input schema requires `experience: integer`, so Apify rejected the request. Fix: set the value as JS number `4`. Dev guideline: when modifying HTTP body parameters in n8n, preserve the JS type (number vs string) — Apify validates against the actor's input schema. Verified by fetching the schema from `/v2/acts/.../builds/last`.
- **v9.12** (2026-05-09): Fixed 3 quality issues identified during execution #39 deep-dive. (1) OpenAI scoring hallucinations — pure Java/Python jobs were scoring 70-90 because the LLM applied boosts based on *resume* content not *job* content. Rewrote `scoreMatch` prompt with mandatory STEP 0 hard caps: Java/Python/Go/C++/Rust roles MUST score 0-35; mobile MUST score 0-35; DevOps/SRE/QA/Data-Eng MUST score 0-35; desktop/WPF MUST score 0-50; jobs with no identifiable backend language MUST score 0-50. matchReason now must quote the specific phrase from the JOB description (not resume) that triggered each boost/penalty. (2) Generic-title high-score jobs ("Cloud Backend Engineer", "Software Development Engineer") with no .NET in description — handled by STEP 0 hard cap E (forces LLM to actually read the description before scoring). (3) Desktop/WPF jobs with .NET in title — added `\bwpf\b`, `\bwinforms\b`, `\bwin32\b`, `\bdesktop\s+(developer|engineer|application)\b` to filter blocklist. Naukri `experience: 5 → 4` to widen 4-7yr range. Expected: smaller but dramatically cleaner final match list (~15-20 jobs, all genuine .NET roles).
- **v9.11** (2026-05-09): Tightened Filter Stack & Quality after analyzing execution #39 (28 final jobs). Root cause: 66% (93/141) of scored jobs scored <50 — wasted ~$0.013 tokens and 2.3 min OpenAI time per run. Broadened title patterns to match Java/Python/Go/C++/kernel anywhere in title (not just `Java Developer`); added Angular/Vue/Laravel/Salesforce/SAP/Temenos/Workday/Genesys/ServiceNow/AVEVA/Labware niche-product blocklist; added test/QA/DevOps/SRE/Network/Security/Support role-type filter; hardened junior pattern (`Software Engineer I`, `SDE-1`) but added `.NET-in-title` override so "Lead Software Engineer I (C#.NET)" still passes; added description-level non-.NET check ("Java is required" / "Python is primary"). Simulated against execution #39: 193 → 94 jobs (was 141), all 4 high-score drops are scoring hallucinations on pure Java/Python jobs, not real matches.
- **v9.10** (2026-05-09): Fixed "Nothing to repeat" crash in `filterStackQuality` — all `\b`, `\s`, `\d` regex escape sequences were stored as literal characters in the live workflow (backslashes dropped during JSON serialization). `/(\d+)\s*\+/` became `/(d+)s*+/` at runtime, a quantifier-on-quantifier error. Title patterns also silently broken (e.g. `/\bjava\s*/` → `/bjava s*/`). Fixed by rewriting node via PowerShell here-string, importing via n8n CLI. Dev note: always use PowerShell here-strings or a file-based approach when writing regex-heavy JS code into n8n — never bash heredocs or JS template literals.
- **v9.9** (2026-05-05): Fixed mode regression introduced in v9.8 — all 9 `runOnceForAllItems` Code nodes reverted to default because a stale pre-v9.7 snapshot was used as base instead of a fresh API fetch. Re-applied all mode fixes from live API. Confirmed `minScore: 70` (user raised from 50 in UI — persisted to node code). Dev guideline added: always fetch fresh from REST API before any workflow edit. Audit script written to verify all node modes, credentials, connections, and cross-node references in one pass.
